...
def main():
    ...
    image_path = '/home/Voldemort/coral/reef/test/images/Screenshot-2025-01-04-161008_png.rf.09742509c0a45467c178fc4280ad7dd6.jpg'  # Change to your image path

    # Check if image exists, if not, create a test image or skip
    if not os.path.exists(image_path):
        print(f"Warning: Test image not found at {image_path}")
        print("Please provide a valid image path or skip to camera detection")

        # Skip to camera detection
        print("\n=== Camera Detection ===")
        print("Starting camera detection...")
        detector.detect_camera(camera_id=0, conf_threshold=0.25)
        return

    result_img = detector.detect_image(image_path, conf_threshold=0.25,
...
# Edge TPU specific function for quick testing
def test_edgetpu_model(model_path, image_path=None):
    """
    Test Edge TPU model with proper error handling
    """
    print(f"Testing Edge TPU model: {model_path}")
    ...
    try:
        ...
        print("✓ Edge TPU library accessible")
        interpreter.allocate_tensors()
        print("✓ Model loaded successfully on Edge TPU")
    except Exception as e:
        print(f"✗ Edge TPU error: {e}")
        print("\nInstall Edge TPU runtime:")
        print("  sudo apt install libedgetpu1-std")
        return

    # Test with YOLO
    try:
        model = YOLO(model_path, task='obb')
        print("✓ YOLO Edge TPU model loaded")

        if image_path and os.path.exists(image_path):
            # Test with correct image size
            results = model(image_path, imgsz=320)
            print(f"✓ Inference successful on {image_path}")
        else:
            print("No test image provided, skipping inference test")

    except Exception as e:
        print(f"✗ YOLO error: {e}")

# Uncomment to test your Edge TPU model
# test_edgetpu_model('/home/Voldemort/coral/reef/newnewmodels/best_full_integer_quant_edgetpu.tflite', 'test_image.jpg')
